...
type CuratedItem = {
  ...
  // optional pin flags (badge only; do NOT affect ordering)
  force?: boolean;
  pinned?: boolean;
  featured?: boolean;
  sticky?: boolean;

  // OPTIONAL per-item CTA override (max 2 rendered)
  cta?: CTA[];
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // Try common locations for curated.json
        ...
        // Normalize and mark ALL as featured (badge only; no reordering)
    ...
    <main style={{ maxWidth: 960, margin: "0 auto", padding: "24px 16px" }}>
      ...
      <ul style={{ listStyle: "none", padding: 0, display: "grid", gap: 12, marginTop: 12 }}>
        {ordered.map((it, i) => {
          ...
          // Per-item override beats defaults; render up to two
          ...
          return (
            <li key={`${it.link}-${i}`} style={{ border: "1px solid #222", borderRadius: 12, padding: 12 }}>
              ...
              <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginTop: 8 }}>
                {ctas.map((c, idx) => (
                  <a key={idx} href={c.href} style={{ ...LINK, fontWeight: 800 }}>
                    {c.label}
                  </a>
                ))}
              </div>
              ...
              <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
                <button onClick={() => navigator.clipboard.writeText(it.link)} style={BTN} title="Copy link">
                  Copy link
                </button>
                <button
                  onClick={() => {
